...
class StockCodeValidator:
    ...
    def validate_stock_code(code: str) -> bool:
        """验证股票代码是否有效"""
        try:
            formatted_code = StockCodeValidator.format_stock_code(code)
            ticker = yf.Ticker(formatted_code)
            
            # 尝试获取基本信息
            info = ticker.info
            
            # 对于中国股票，Yahoo Finance可能没有完整数据
            # 我们主要检查是否能获取到基本的价格数据
            hist = ticker.history(period="5d")
            
            # 如果能获取到价格数据或者有基本信息，认为代码有效
            return not hist.empty or bool(info.get('symbol', False))
        except Exception as e:
            print(f"验证股票代码 {code} 时出错: {str(e)}")
            return False
...
class DataManager:
    ...
    def __init__(self, stock_code: str, data_dir: str = "data"):
        self.stock_code = StockCodeValidator.format_stock_code(stock_code)
    ...
    def fetch_stock_data(self, period: str = "2y") -> bool:
        """获取股票数据"""
        try:
            ...
            if hist.empty:
                print(f"警告: 无法获取 {self.stock_code} 的价格数据")
                return False
            ...
            with open(f"{self.data_dir}/{self.stock_code.replace('.', '_')}_price.json", 'w', encoding='utf-8') as f:
            ...
            with open(f"{self.data_dir}/{self.stock_code.replace('.', '_')}_info.json", 'w', encoding='utf-8') as f:
                json.dump(info_data, f, ensure_ascii=False, indent=2, default=str)
            ...
            try:
                ...
                if not financials.empty:
                    financial_data = {
                        'symbol': self.stock_code,
                        'data': financials.reset_index().to_dict('records')
                    }
                    
                    with open(f"{self.data_dir}/{self.stock_code.replace('.', '_')}_financial.json", 'w', encoding='utf-8') as f:
                        json.dump(financial_data, f, ensure_ascii=False, indent=2, default=str)
            except:
                print(f"警告: 无法获取 {self.stock_code} 的财务数据")
            
            print(f"✓ {self.stock_code} 数据获取完成")
            return True
            
        except Exception as e:
            print(f"获取 {self.stock_code} 数据时出错: {str(e)}")
            return False
    
    def load_data(self) -> bool:
        """加载本地数据"""
        try:
            price_file = f"{self.data_dir}/{self.stock_code.replace('.', '_')}_price.json"
            info_file = f"{self.data_dir}/{self.stock_code.replace('.', '_')}_info.json"
            financial_file = f"{self.data_dir}/{self.stock_code.replace('.', '_')}_financial.json"
...
class MultiStockAnalyzer:
    ...
    def analyze_single_stock(self, stock_code: str, force_refresh: bool = False) -> bool:
        ...
        try:
            ...
            # 验证股票代码（但即使验证失败也尝试分析）
            is_valid = StockCodeValidator.validate_stock_code(stock_code)
            if not is_valid:
                print(f"⚠️  股票代码 {stock_code} 验证失败，但将尝试进行分析...")
            else:
                print(f"✓ 股票代码 {stock_code} 验证通过")
            
            # 数据管理
            data_manager = DataManager(stock_code)
            ...
            if not data_loaded:
                if not data_manager.fetch_stock_data():
                    return False
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='多股票量化分析系统')
    parser.add_argument('--stocks', '-s', nargs='+', help='股票代码列表 (例如: 002702.SZ 000001.SZ)')
    parser.add_argument('--file', '-f', help='包含股票代码的文件路径 (每行一个股票代码)')
    parser.add_argument('--refresh', '-r', action='store_true', help='强制刷新数据')
    ...
    if args.stocks:
        ...
        # 默认示例股票
        stock_codes = ['002702.SZ', '000001.SZ', '600036.SS']
        print("未指定股票代码，使用默认示例: 002702.SZ, 000001.SZ, 600036.SS")
    ...
    for code in stock_codes:
        ...
        print(f"股票代码: {code} -> {formatted_code}")
    
    # 创建分析器并执行分析
    analyzer = MultiStockAnalyzer(args.output)
    
    if len(formatted_codes) == 1:
        return analyzer.analyze_single_stock(formatted_codes[0], args.refresh)
    else:
        return analyzer.analyze_multiple_stocks(formatted_codes, args.refresh)
